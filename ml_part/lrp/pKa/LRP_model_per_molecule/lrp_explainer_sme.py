...
class PkaLRP:
    def __init__(self,
        ...
        self.node_relevances, self.edge_relevances = self.lrp(self.bg, self.node_features, self.edge_features, self.atoms_groups)
        if is_centrize_relevances:
            self.node_relevances, self.edge_relevances = normalize_to_minus_one_to_one(data_node=self.node_relevances, data_edge=self.node_relevances)

        self.relevance_fluorine_group = PkaLRP.calculate_relevance_for_f_group_in_molecule(
    ...
    def lrp(self, bg, node_feats, edge_feats, atoms_subgroups):
        self.model.eval()
        ...
        for atoms_subgroup in atoms_subgroups:
            ...
            # e0 = x_edge
            # mask_edge = torch.zeros(x_edge.shape).to(self.device)
            # for mol_edge_idx in range(self.mol.GetNumBonds()):
            #     mol_begin_node = self.mol.GetBonds()[mol_edge_idx].GetBeginAtomIdx()
            #     mol_end_node = self.mol.GetBonds()[mol_edge_idx].GetEndAtomIdx()

            #     if mol_begin_node in atoms_subgroup and mol_end_node in atoms_subgroup:
            #         mask_edge[mol_edge_idx] = 1
            # x_edge = x_edge * mask_edge + (1 - mask_edge) * x_edge.data
            
            # forward
            node_feats = self.model.init_context(bg, x_node, edge_feats_original)
            ...
            for atom_index in atoms_subgroup:
                all_node_relevances[atom_index] = (h0.data * h0.grad).data.sum().item()
            h0.grad.data.zero_()
    ...
    def obtain_subgroups_from_mol(self, molecule):
        ...
        if self.fluorine_group == "non-F":
            nitrogen_atom_id = PkaLRP.get_atom_id_by_symbol(mol=molecule, atom_symbol='N')
            if nitrogen_atom_id != None:
                atom_id, distance = find_the_furthest_atom(mol=molecule,
                                                        atom_id=nitrogen_atom_id)
                matches = [[nitrogen_atom_id, atom_id]]
            # matches = []
    ...
    def calculate_relevance_for_f_group_in_molecule(mol, fluorine_group,
        ...
        relevance, number_of_relevances = 0, 1
        ...
        if "non" in fluorine_group:
            ...
            return round((node_relevances[atom_id]), 3)
        ...
        fluorine_relevance, number_of_fluorine_relevance = 0, 0
        for unique_atom_index in unique_atoms:
            fluorine_relevance += node_relevances[unique_atom_index]
            number_of_fluorine_relevance += 1

        relevance_per_group = [node_relevances[match[0]] * len(match) for match in f_group_matches]

        relevance += (fluorine_relevance / number_of_fluorine_relevance)
