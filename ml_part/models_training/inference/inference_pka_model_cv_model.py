...
def load_pKa_acidic_model(args):
    ...
    # combined data from acid and amine molecules, cross val 1 model
    pka1_model.load_state_dict(torch.load(r'ml_part\weights\pKa\combined_dataset\cv_models\acidic\cv_0_best_loss_lr_0.0007452146740113421_wd_0.0025791056042483687_train_type_predictor_and_readout.pkl',map_location='cpu'))
    return pka1_model
...
def train(model, _test_set):
    ...
    for index in range(len(true_val_values)):
        true_value_to_pred[round(true_val_values[index], 2)] = round(pred_val_values[index], 2)
    ...
    print(true_value_to_pred)
    print(val_metrics)

    return pred_val_values, true_val_values
...
if __name__ == '__main__':
    ...
    df = train_set_basic[train_set_basic['fold_id'] == 1]

    test_set = load_dataset(args,df,"test")

    pred_val_values, true_val_values = train(acidic_model, test_set)
    index = 0
    SMILES_to_pKa = {}
    for _, row in df.iterrows():
        # print(row['Smiles'], row['pKa'], true_val_values[index], pred_val_values[index])
        SMILES_to_pKa[row['Smiles']] = (row['pKa'], pred_val_values[index])
        index += 1
