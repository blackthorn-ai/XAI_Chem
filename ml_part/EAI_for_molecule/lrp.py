...
class Lrp:
    def __init__(self, logP_model, amine_pKa_model, acid_pKa_model, smiles, functional_group, exp_config, args, model_type=ModelType):
    ...
    @staticmethod
    def normilize_relevances(relevance_dict):
        relevance_array = [atom_relevance for atom_id, atom_relevance in relevance_dict.items()]
        max_relevance = max(relevance_array)
        min_relevance = min(relevance_array)

        for atom_id, atom_relevance in relevance_dict.items():
            relevance_dict[atom_id] = (atom_relevance - min_relevance) / (max_relevance - min_relevance)

        return relevance_dict

    def create_mol_as_bigraph(self):
        mol = Chem.MolFromSmiles(self.smiles)
        mol_subgroup = Chem.MolFromSmiles(self.subgroup_smiles)

        new_order = rdmolfiles.CanonicalRankAtoms(mol)
        mol = rdmolops.RenumberAtoms(mol, new_order)

        return mol, mol_subgroup
    ...
    def save_molecule_with_relevances(self, output_svg_path): 
        ...
        mol, mol_subgroup = self.create_mol_as_bigraph()

        drawer = rdMolDraw2D.MolDraw2DSVG(1200, 1200)
        atoms_subgroups = mol.GetSubstructMatches(mol_subgroup)
